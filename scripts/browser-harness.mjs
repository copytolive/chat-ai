...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/status') return send(res, 200, 'application/json', JSON.stringify({ ok: true, release: 'browser-test', automation: { enabled: true }, whatsapp: { provider: 'cloud', connection: 'ready', configured: true, accountPaired: true }, ai: { enabled: true, configured: true, model: 'mock-primary', fallbackConfigured: true, fallbackModel: 'mock-fallback' }, marketing: { enabled: true, configured: true, activeSessions: 1, verifiedKnowledgeFacts: 12, sessionPersistence: 'durable-json-single-instance' }, metrics: { messagesReceived: 21, repliesSent: 20, failures: 0, handoffs: 2, optOuts: 1, p95LatencyMs: 384 } }))
