...
function renderStatus(payload) {
  ...
  els.queue.textContent = wa.provider === 'cloud' ? (queue.encrypted ? `ENCRYPTED · ${queue.pending || 0} PENDING` : 'NOT ENCRYPTED') : (wa.sessionPersistence ? 'SESSION PERSISTENT' : 'N/A')
  els.automation.textContent = payload.automation?.enabled ? 'AUTO-RUN ENABLED' : 'KILL SWITCH OFF'
  ...
  els.handoff.textContent = handoff.configured ? `READY · ${handoff.localInbox ? 'LOCAL INBOX' : 'SIGNED WEBHOOK'} · ${handoff.queue?.encrypted ? 'ENCRYPTED' : 'PLAIN'}` : 'NOT CONFIGURED'
  els.knowledge.textContent = `${marketing.verifiedKnowledgeFacts || 0} VERIFIED FACTS`
  els.persistence.textContent = wa.provider === 'baileys' && wa.sessionPersistence ? `WA ${wa.sessionPersistence} · ${label(marketing.sessionPersistence || 'state')}` : label(marketing.sessionPersistence || 'unknown')
...
async function refreshChannel(status) {
  ...
  if (status?.whatsapp?.connection === 'open') { lastQr = null; els.qrWrap.innerHTML = '<div class="success">Connected · Auto-run</div>'; els.qrHelp.textContent = 'QR pairing is complete. The session is saved on durable storage, automatic replies are active, and reconnect runs automatically after restart.'; return }
  if (!status?.whatsapp?.hasQr) { lastQr = null; els.qrWrap.innerHTML = '<div class="placeholder">Waiting for a fresh QR…</div>'; els.qrHelp.textContent = 'The scanner starts automatically. When the QR appears, open WhatsApp → Linked devices → Link a device.'; return }
  const { response, payload } = await requestJson('/qr'); if (!response.ok || !payload.qr || payload.qr === lastQr) return; lastQr = payload.qr; const img = document.createElement('img'); img.src = payload.qr; img.alt = 'WhatsApp pairing QR code'; img.width = 360; img.height = 360; els.qrWrap.replaceChildren(img); els.qrHelp.textContent = 'Scan once with WhatsApp → Linked devices. After pairing, this page switches to CONNECTED and the agent operates automatically.'
}
